...
// Handlers
const { handleStart, handleBackup, handleStatsCommand, handlePing } = require('./handlers/startHandler');
...
bot.setMyCommands([
    ...
    { command: 'backup', description: '💾 Backup database (Admin)' },
    ...
    { command: 'addkey', description: '🔑 Tambah license key (Admin)' },
    { command: 'listkey', description: '📋 Lihat license keys (Admin)' },
    { command: 'delkey', description: '🗑️ Hapus license key (Admin)' },
    { command: 'notif', description: '📢 Kirim notifikasi ke web (Admin)' },
...
bot.onText(/\/backup/, async (msg) => {
    const { handleBackup } = require('./handlers/startHandler');
    await handleBackup(bot, msg);
});

// ==================== PING COMMAND ====================
bot.onText(/\/ping/, async (msg) => {
    const { handlePing } = require('./handlers/startHandler');
    await handlePing(bot, msg);
});

// ==================== ADMIN: STATS COMMAND ====================
bot.onText(/\/stats/, async (msg) => {
    if (!isAdmin(msg.chat.id)) return;
    const { handleStatsCommand } = require('./handlers/startHandler');
    await handleStatsCommand(bot, msg);
});
...
bot.on('callback_query', async (query) => {
    ...
    if (query.data.startsWith('bc_')) {
        if (!isAdmin(query.from.id)) {
            await bot.answerCallbackQuery(query.id, { text: '❌ Tidak ada akses' });
            return;
        }
        ...
        if (!bc || bc.adminId !== query.from.id) {
            await bot.answerCallbackQuery(query.id, { text: '⚠️ Session expired', show_alert: true });
            return;
        }
...
${failed > 0 ? `\n⚠️ <i>${failed} users telah dihapus (blocked/deleted)</i>` : '🎉 <i>Semua pesan terkirim dengan sukses!</i>'}
    ...
    // Handle other callbacks (from colorHandler, etc.)
